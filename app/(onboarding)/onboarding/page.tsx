...
const OnboardingPage: NextPageWithLayout = observer(() => {
  ...
  const { data: workspaces } = useSWR(
    ...
    async () => await workspaceService.userWorkspaces(),
    {
  ...
  useEffect(() => {
    const handleStepChange = async () => {
      ...
      if (
        ...
        !onboardingStep.workspace_join &&
        !onboardingStep.workspace_create &&
        !onboardingStep.workspace_information
      ) {
        setStep(1);
      }
      ...
      if (
        onboardingStep.profile_complete &&
        (onboardingStep.workspace_create || onboardingStep.workspace_join) &&
        onboardingStep.workspace_information
      ) {
        await updateCurrentUser({
          isOnbordered: true,
    ...
    <>
      <section className="h-full overflow-auto rounded-t-md bg-custom-background-100 px-0 pb-56 md:px-7">
        {user && step !== null && workspaces ? (
          <>
            <div className="absolute top-12 right-36 text-white">
            ...
            <div className="flex w-full">
              {step === 1 ? (
                ...
                <UserDetails
                  setUserName={(value) => setValue("name", value)}
                  user={user}
                  workspacesList={workspacesList}
                />
              ) : (
                <WorkspaceInformation
                  finishOnboarding={function (): Promise<void> {

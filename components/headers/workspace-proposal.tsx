...
import { AddProposalDialog } from "../proposal/add/dialog";

export const ProposalsHeader = observer(({ searchValue, setSearchValue, onProposalAdded }:any) => {
  return (
    <div className="relative z-10 flex h-[3.75rem] w-full flex-shrink-0 flex-row items-center justify-between gap-x-2 gap-y-4 border-b border-custom-border-200 p-4">
          ...
          <Breadcrumbs>
            <Breadcrumbs.BreadcrumbItem
              ...
              link={
                <BreadcrumbLink
                  label="Propostas"
                  icon={<BriefcaseBusiness className="h-4 w-4 text-custom-text-300" />}
                />
              }
            />
          </Breadcrumbs>
      ...
      <div className="flex w-full max-w-xs items-center justify-end gap-3 sm:max-w-md">
        ...
        <AddProposalDialog onProposalAdded={onProposalAdded}/>
      </div>
    </div>
  );
});
...
import { getUser, createUser, getMe } from "./user.action";
...
export async function oAuth({ medium, credential, clientId }: any) {
  if (medium === "google") {
    ...
    if (!googleUser || typeof googleUser === "undefined") {
      throw new Error("");
    }
...
export async function validateOtpAndSignIn({
  email,
  imputedOTP,
}: {
  email: string;
...
export async function reSignIn(_request: NextRequest) {
  try {
    const currentUser = await getMe(_request);

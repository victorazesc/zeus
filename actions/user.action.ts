...
export async function setPassword(req: NextRequest, password: string) {
  try {
    ...
    await prisma.user.update({
      ...
      data: { password: hash, isAccessPassword: true },
    });
    return true;
  } catch (error: any) {
    throw new Error(error);
...
export async function retrieveUserSettings(
  req: NextRequest
  ...
  try {
    ...
    const getUserLastWorkspace = async () => {
      return await prisma.workspace.findUnique({
        where: { id: user.lastWorkspaceId as number },
      });
    };
...
export async function getUsers(
  ...
  const users = prisma.user.findMany({
    where: { lastWorkspaceId: currentUser.lastWorkspaceId },
  });
